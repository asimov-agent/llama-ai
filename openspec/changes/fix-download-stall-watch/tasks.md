...
- [x] 2. Implement the real stall-watch in `scripts/hf_download.py`:
...
- [x] 3. Add hermetic tests (no skips) to `tests/`:
...
- [x] 4. Verify: `make lint`, `make test-unit`; `make test-top-tier` (container);

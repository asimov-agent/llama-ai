...
- [ ] 1. OpenSpec change scaffold (proposal.md + specs/llama-ai-tooling/spec.md + tasks.md) —
      VALID — committed + pushed to `feat/feat-top-tier-download-top-tier-family-keyword-top`
      before implementation.
- [ ] 2. Add `_family_gguf_repos(keyword)` to `scripts/llama_serve.py`: full model-set search
...
- [ ] 3. Extend `discover_top_tier(..., family=None)`: when `family` is set, the repo list
...
- [ ] 4. CLI: add `--family` to `main()`'s parser; pass through in `_main_download_top_tier`;
      dry-run/list readout gains `[family] <kw> → N providers (M files) match`; zero matches
      → `no GGUF repos found for family '<kw>'` + exit 1.
- [ ] 5. Hermetic unit tests in `tests/test_llama_ai.py` (`make test-unit`):
...
- [ ] 6. Acceptance tests in `tests/test_top_tier_acceptance.py` (`make test-top-tier` / CI):

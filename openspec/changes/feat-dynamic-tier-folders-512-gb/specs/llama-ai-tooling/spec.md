...
includes the VRAM-parameterized placement tests) and `make test-top-tier` (acceptance +
real-CLI), so the placement guarantee is continuously verified, not just locally.

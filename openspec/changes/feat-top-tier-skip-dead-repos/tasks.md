...
- [ ] 2. Add a **pre-flight downloadability probe** `_probe_file_downloadable(repo, filename)`
...
- [ ] 3. In `discover_top_tier`, run the probe on each candidate's file, **exclude** any that
      fail, and log `[top-tier] skipping <repo>::<file>: <reason>` (no 3× retry).
- [ ] 4. **Refill**: when repos are gated/dead, keep walking the trending list (widening the
      window) until `limit` valid candidates are collected.
- [ ] 5. Add a **completion summary** `downloaded N/M (+S skipped: access-denied, dead)`.
- [ ] 6. Add hermetic unit tests:
...
- [ ] 7. Add real-HF tests: `unsloth/...` probes `ok` (real GGUF chunk → 200 auth-ok),
